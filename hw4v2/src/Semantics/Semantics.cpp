...
void Semantics::checkOperandsOfSameType(Exp *exp) const
{
    ...
    {
        Emit::error(exp->getLineNum(), "'" + sym + "' requires operands of the same type but lhs is type " + lhsData->stringify() + " and rhs is type " + rhsData->stringify() + ".");
    }
...
void Semantics::checkOperandsOfType(Exp *exp, const Data::Type type) const
{
    ...
    {
        Emit::error(exp->getLineNum(), "'" + sym + "' requires operands of type " + typeString + " but lhs is of type " + lhsData->stringify() + ".");
    }

    if (rhsData->getType() != type)
    {
        Emit::error(exp->getLineNum(), "'" + sym + "' requires operands of type " + typeString + " but rhs is of type " + rhsData->stringify() + ".");
    }
...
void Semantics::checkUnaryOperands(const Unary *unary) const
{
    ...
    switch (unary->getType())
    {
        ...
        case Unary::Type::Question:
            ...
            {
                Emit::error(unary->getLineNum(), "Unary '" + unary->getSym() + "' requires an operand of type int but was given type " + lhsData->stringify() + ".");
            }
            break;
        ...
        case Unary::Type::Not:
            ...
            {
                Emit::error(unary->getLineNum(), "Unary '" + unary->getSym() + "' requires an operand of type bool but was given type " + lhsData->stringify() + ".");
            }
            if (lhsData->getIsArray())
            {
...
void Semantics::checkUnaryAsgnOperands(const UnaryAsgn *unaryAsgn) const
{
    ...
    switch (unaryAsgn->getType())
    {
        ...
        case UnaryAsgn::Type::Dec:
            ...
            if (lhsData->getType() != Data::Type::Int)
            {
                Emit::error(unaryAsgn->getLineNum(), "Unary '" + unaryAsgn->getSym() + "' requires an operand of type int but was given type " + lhsData->stringify() + ".");
            }
            break;
        default:
...
void Semantics::checkIndex(const Binary *binary) const
{
    ...
    {
        Emit::error(binary->getLineNum(), "Array '" + arrayId->getName() + "' should be indexed by type int but got type " + indexData->stringify() + ".");
    }
...
void Semantics::leaveScope()
{
    ...
    for (auto const& [name, voisIdode] : syms)
    {
        ...
        if (isVar(decl))
        {
            ...
            {
                Emit::warn(var->getLineNum(), "The variable '" + var->getName() + "' seems not to be used.");
            }
        }
        else if (isParm(decl))
        {
            ...
            {
                Emit::warn(parm->getLineNum(), "The variable '" + parm->getName() + "' seems not to be used.");
            }
        }
    }
...
Data * Semantics::setAndGetExpData(const Exp *exp) const
{
    ...
    switch (exp->getNodeKind())
    {
        ...
        case Node::Kind::Call:
        {
            ...
            if (prevDecl != nullptr && prevDecl->getNodeKind() != Node::Kind::Var)
            {
        ...
        case Node::Kind::Id:
        {
            ...
            if (prevDecl != nullptr && prevDecl->getNodeKind() != Node::Kind::Func)
            {
